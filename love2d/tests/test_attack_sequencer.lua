...
describe("Real attack files", function()
    ...
    it("previously partial attacks are now ready", function()
        assert_eq(AttackParser.getAttackStatus("sans_bonestab1"), "ready")
        assert_eq(AttackParser.getAttackStatus("sans_final"), "ready")
        assert_eq(AttackParser.getAttackStatus("sans_intro"), "ready")
        assert_eq(AttackParser.getAttackStatus("sans_multi2"), "ready")
        assert_eq(AttackParser.getAttackStatus("sans_multi3"), "ready")
    end)
...
describe("Bone repeat", function()
    it("BoneVRepeat spawns Count vertical bones spaced by Spacing", function()
        ...
        -- dir 0: offset runs along -X (original: StartX - cos(0)*spacing*i)
        assert_eq(Stubs.FakeBone.spawned[4].x, -50, "fourth at startX - 3*spacing")
    ...
    it("BoneHRepeat spawns Count horizontal bones spaced down Y", function()
        ...
        -- dir 1: offset runs along -Y (original: StartY - sin(90)*spacing*i)
        assert_eq(Stubs.FakeBone.spawned[3].y, -376, "third at startY - 2*spacing")
